...
import {FormStyled, FieldWrapper, Label, Input, Error, DetailWrapper, SubmitButton, } from "./Payment.styled";
const visaSchema = Yup.object().shape({
    cardNumber: Yup.string().required("Required"),
    cardHolder: Yup.string().required("Required"),
    expiryDate: Yup.string().required("Required").matches(/^[0-9]{2}\/[0-9]{2}$/, "Expiry date is not valid") // Kiểm tra chuỗi có đúng định dạng "MM/YY" hay không
    .test({
      name: "expiryDate",
      test: (value) => {
                ...
                <FormStyled>
                    ...
                    <DetailWrapper>
                        ...
                        <div style={{display:"flex", justifyContent:"space-between"}}>
                            ...
                            <div>
                                <p style={{textAlign:"end"}}>${paymentDetails.flightPrice}</p>
                                <p style={{textAlign:"end"}}>${paymentDetails.insurancePrice}</p>
                                <p style={{textAlign:"end"}}>${paymentDetails.serviceFee}</p>
                            </div>
                        </div>
                        ...
                        <div style={{display:"flex", justifyContent:"space-between"}}>
                            ...
                            <div>
                                <h3 style={{textAlign:"end"}}>${paymentDetails.flightPrice + paymentDetails.insurancePrice + paymentDetails.serviceFee}</h3>
                            </div>
                        </div>
                    </DetailWrapper>
                    <p style={{fontSize:"14px", color:"#1C2668", fontWeight:"600"}}>By clicking Pay Now, I agree that I have read and accepted the <span style={{color:"#2FA4FF"}}>Terms and Conditions</span> of FLIGO</p>
